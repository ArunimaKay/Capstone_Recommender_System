...
freq_dist = FreqDist(word for word in brown.words())
...
def get_log_prob(text):
    prob = float(freq_dist[text.lower()])/float(freq_dist.N())
...
print("Brown probability of text 'example' is {}".format(get_log_prob('example')))
...
def getMetadataDF():
    ...
    print("Loading related metadata dataset (Toys-filtered version):")
    print()
    # file = 'metadata.json.gz'
    #file = './data/meta_Toys_and_Games.json.gz'
    file = './data/filtered_Toys_meta.json.gz'
    start = time.time()
    md = getDF(file)
...
# Functions for use in other processes
...
def derive_token_text(token):
    text_val = token.lemma_.lower()

    if (text_val is None) or (len(text_val) == 0):
        text_val = token.norm_.lower()
...
def get_word_scores(text_input, doc):
    ...
    tokenlist = [token for token in doc if derive_token_text(token)  not in stopWords]
    wordlist = [derive_token_text(token)  for token in tokenlist]
    wordfreq = []
    wordset = set(wordlist)
    for word in wordset:
        wordfreq.append((word, np.log(1.0 + float(1+wordlist.count(word)))))
    ...
    for token in doc:
        if token not in stopWords:
            lower_norm = derive_token_text(token) 
            #tf_pair = term_freq_dict.get(lower_norm)
            weight = weights_map.get(lower_norm)
            if (weight is not None) and (lower_norm not in seen_tokens):
                word_scores[lower_norm]=np.log(weight)
...
def preprocess_doc(doc):
    ...
    for token_ind in range(len(doc)):
        ...
        token = doc[token_ind]
        this_text = derive_token_text(token) 
        if debug:
...
def get_lemmatized_token(token):
    ...
    for token in [token]:
        if (token.lemma_ not in IGNORED_LEMMAS) and (token.pos_ not in IGNORED_POS) and token.text.isalpha():            
            ...
            derived_text = derive_token_text(token)
            if derived_text is None:
                if debug:
                    print("!!!derived text for token {} is None".format(token.text))
                continue

            # check if this token contains 2 or more sequential punctuation characters
            if check_embedded_punctuation(derived_text):
                stop_count += 1
                if debug:
                    print("!!!token {} is a stop word".format(token.text))
                continue
            else:  
                non_stop_count += 1
            ...
            #else:
            if english_dict.check(derived_text):
                processed_text.append(derived_text)
            else:
                if debug:
                    print("That's not English")
    
    if non_stop_count > 3:
        return "", np.zeros(300)
        if debug:
                    print("!!!! Returning empty string and zeros vector...more than 3 stop words")
    else:
        if (non_stop_count > 0) and (stop_count > 0):
            vector = np.divide(vector, non_stop_count)
    
    if debug: 
        print("Returning processed_text '{}' and vector {}".format(processed_text, vector))
...
if debug:
    ...
    doc = nlp(test_str)
    for sent in doc.sents:
        print("...processing sentence '{}'...".format(sent.text))
        for token in sent:
            print("....processing token '{}'...".format(token.text))
            #yield chunk.text, chunk.vector
            lemmatized_text, vect = get_lemmatized_token(token)
            print(".......lemmatized_text = '{}'".format(lemmatized_text))
...
def write_vectors(word_vects, out_m, out_tf_features, product, rating, processed_text, text_score, concept_vec):
    """Write product, rating, phrase and sense vector to metadata and vectors files"""
    phrase = processed_text
    sense_vector = concept_vec

    print(','.join([product, str(rating), str(text_score), phrase]), file=out_tf_features)
...
def write_product_rev_features(out_f, product, rating, processed_text, text_score, concept_vec):
    ...
    phrase = processed_text
    sense_vector = concept_vec
    print(','.join([product, str(rating),text_score,phrase]), file=out_f)


# Parse the product reviews dataset
df = getReviewsDF()

print("Parsed product reviews dataset...")
print(df.shape)
print(df.columns)
df.head(2)
...
cat_item_build_set = [
    ["Toys & Games|Baby & Toddler Toys", "Toys & Games|Tricycles, Scooters & Wagons"],
    ["Toys & Games|Stuffed Animals & Plush"],
    ["Toys & Games|Arts & Crafts", "Toys & Games|Hobbies"],
    ["Toys & Games|Dolls & Accessories"],
    ["Toys & Games|Action Figures & Statues"],
    ["Toys & Games"],
    ["Toys & Games|Novelty & Gag Toys", "Toys & Games|Dress Up & Pretend Play"],
    ["Toys & Games|Electronics for Kids", "Toys & Games|Toy Remote Control & Play Vehicles"],
    ["Toys & Games|Puzzles", "Toys & Games|Building Toys"],
    ["Toys & Games|Sports & Outdoor Play", "Sports & Outdoors|Boating & Water Sports", "Sports & Outdoors|Team Sports"],
    ["Software|Lifestyle & Hobbies", "Toys & Games|Party Supplies", "Toys & Games|Grown-Up Toys", "Health & Personal Care|Stationery & Party Supplies"],
    ["Software|Education & Reference", "Toys & Games|Learning & Education"],
    ["Toys & Games|Games"]
]
...
products_in_5 = pd.read_csv('./data/products_in_5.csv', names=['asin'])
print("\nproducts_in_5({}):\n".format(len(products_in_5)), products_in_5[:5])

product_buildset = pd.read_csv('./data/cluster_build_sets.filtered.csv', names=['asin','build_set'])
...
#print("\nmd_filtered({}):\n".format(len(md_filtered)), md_filtered[:5])
...
for token_ind in range(len(weights_df)):
    term = weights_df['term'].iloc[token_ind]
    weight = weights_df['weight'].iloc[token_ind] 
    weights_map[term] = weight

print("weights_map:")

term_count = 0
for term, weight in weights_map.items():
    print(term, weight)
    term_count += 1
    if term_count > 10:
        break
...
# Otherwise, these item are already written out to the files in metadata_filepath and vectors_filepath, using PyTables.
    ...
    sample_vect = [vec for _, _, vec in get_vectors("example text can be useful", nlp)]
    vect_dim = 300
    ...
    with open_file(vectors_filepath, mode="w", title="Word Vectors") as out_v:
        ...
        atom = tables.Float32Atom(vect_dim)
        shape = (0,)
        filters = tables.Filters(complevel=5, complib='zlib')
        word_vect = out_v.create_earray(out_v.root, 'vector', atom, shape, filters=filters)

    with open(metadata_filepath, 'w') as out_m, open(tf_features_filepath, 'w') as out_tf_features, open_file(vectors_filepath, mode="a", title="Word Vectors") as out_v:
            ...
            for iter_ind in range(this_iteration_size):
                ...
                for processed_text, text_score, concept_vec in get_vectors(reviewText, nlp):
                    ...
                    if (len(processed_text)>0):
                        ...
                        write_vectors(out_v.root.vector, out_m, out_tf_features, product, rating, processed_text, text_score, concept_vec)
                        vectors_count += 1
...
# In[ ]:
    ...
    # Save the HDBScan model with a name indicating the number of word vectors clustered
        with open('./data/hdbscanner.prod_att.{}.pickle'.format(build_set), 'wb') as pickle_file:
...
# In[29]:
    ...
    for i, c in enumerate(clusters):
        ...
        # Look for clusters where the members outnumber the exemplars by 1.5 times
        if ((exemplars_len>0) and (len(members)>(1.5*exemplars_len))):
